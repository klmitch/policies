...
class TestParseRule(tests.TestCase):
    ...
    rules = [
        ...
        ('a / b', Instructions([Ident('a'), Ident('b'), true_div_op,
                                set_authz])),
        ('a // b', Instructions([Ident('a'), Ident('b'), floor_div_op,
                                 set_authz])),
        ...
        ('a + -2', Instructions([Ident('a'), Constant(-2), add_op,
                                 set_authz])),
        ('a << b', Instructions([Ident('a'), Ident('b'), left_shift_op,
                                 set_authz])),
        ('a >> b', Instructions([Ident('a'), Ident('b'), right_shift_op,
                                 set_authz])),
        ('a & b', Instructions([Ident('a'), Ident('b'), bit_and_op,
                                set_authz])),
        ('a ^ b', Instructions([Ident('a'), Ident('b'), bit_xor_op,
                                set_authz])),
        ('a | b', Instructions([Ident('a'), Ident('b'), bit_or_op,
                                set_authz])),
        ('a in b', Instructions([Ident('a'), Ident('b'), in_op, set_authz])),
        ('a not in b', Instructions([Ident('a'), Ident('b'), not_in_op,
                                     set_authz])),
        ('a is b', Instructions([Ident('a'), Ident('b'), is_op, set_authz])),
        ('a is not b', Instructions([Ident('a'), Ident('b'), is_not_op,
                                     set_authz])),
        ...
        ('a if b else c', Instructions([Ident('b'), Ident('a'), Ident('c'),
                                        trinary_op, set_authz])),
        ('{}', Instructions([Constant(frozenset()), set_authz])),
        ('{,}', Instructions([Constant(frozenset()), set_authz])),
        ('{a}', Instructions([Ident('a'), SetOperator(1), set_authz])),
        ('{a, b, c}', Instructions([Ident('a'), Ident('b'), Ident('c'),
                                    SetOperator(3), set_authz])),
        ('{a, b, c,}', Instructions([Ident('a'), Ident('b'), Ident('c'),
                                     SetOperator(3), set_authz])),
        ...
        ('a(,)', Instructions([Ident('a'), CallOperator(1), set_authz])),
        ('a(1)', Instructions([Ident('a'), Constant(1), CallOperator(2),
                               set_authz])),
        ('a(1, 2, 3)', Instructions([Ident('a'), Constant(1), Constant(2),
                                     Constant(3), CallOperator(4),
                                     set_authz])),
        ('a(1, 2, 3,)', Instructions([Ident('a'), Constant(1), Constant(2),
                                      Constant(3), CallOperator(4),
                                      set_authz])),
        ('True {{}}', Instructions([Constant(True), set_authz])),
        ('True {{,}}', Instructions([Constant(True), set_authz])),
        ('True {{a=1}}', Instructions([Constant(True), set_authz,
